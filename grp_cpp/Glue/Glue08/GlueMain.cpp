...
static double EndTime = 400.0;  // make this value visible to other functions
...
void main(void)
    ...
    LoadRobot = new CLoadRobot("LoadRobot",3.5,3.1,2.5,PROCESS_A);
    UnloadRobot1 = new CUnloadRobot("UnloadRobot1",4.1,2.2,PROCESS_A);
    UnloadRobot2 = new CUnloadRobot("UnloadRobot2",4.1,2.2,PROCESS_A);
    GlueApplicator = new CGlueApplicator("GlueApplicator",10.4,3.5,PROCESS_A);
    #endif //SIMULATION
    ...
    */
    ...
    // Transport tasks
    Transport1 = new CTransport("Transport1",1,BeltMotion1,Clamp1,
      UnloadRobot1,PROCESS_A);
    Transport2 = new CTransport("Transport2",2,BeltMotion2,Clamp2,
      UnloadRobot2,PROCESS_A);
        // Make last WAIT long enough so all action finishes in both
    // Assmebly Task
    Assembly = new CAssembly("Assembly",PROCESS_A);
    ...
    if(ThisProcess == PROCESS_A)LowPriority->Append(Assembly);
    ...
    if(ThisProcess == PROCESS_A)LowPriority->Append(LoadRobot);
    if(ThisProcess == PROCESS_A)LowPriority->Append(UnloadRobot1);
    if(ThisProcess == PROCESS_A)LowPriority->Append(UnloadRobot2);
    if(ThisProcess == PROCESS_A)LowPriority->Append(GlueApplicator);
    #endif //SIMULATION
