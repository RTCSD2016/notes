...
CDataLogger::CDataLogger(char *name,int ProcNo)  // Constructor
    ...
    {
    DataOutBelt = new DataOutput(6,0.5,200.0,0.1);  // Start at
        // t = 0.5 to avoid initial transients
    DataOutOven = new DataOutput(6,0.5,200.0,2.0);
...
int CDataLogger::Run(void)
   ...
   // to store data
     ...
     if(DataOutBelt->IsTimeForOutput())
        {
        double x1set,x1,x2set,x2,v1,v2;
        ...
        CriticalSectionEnd();
        DataOutBelt->AddData(x1set,x1,x2set,x2,v1,v2,END_OF_ARGS);
